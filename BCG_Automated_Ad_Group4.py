...
query_text = 'perfect' #Fetching posts for the desired keyword mentioned by the user
...
all_data_df = []
all_data_df = pd.DataFrame(all_data)
all_data_filename = 'data_complete.csv'
...
X_new = pd.read_csv('data_complete.csv',sep=",")


# In[461]:


X_new['price']=X_new['price'].str.replace('$','')
X_new['bathroom']=X_new['bathroom'].str.replace('+','')
X_new['bathroom']=X_new['bathroom'].str.replace('split','')
X_new['bathroom']=X_new['bathroom'].str.replace(' ','')
...
X_new['reposted'] = np.where(X_new['post_id_old'].isnull, 0, 1)
